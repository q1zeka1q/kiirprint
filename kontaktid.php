...
// Страховочный массив для новых слов (на случай, если не успеешь добавить в словари)
...
// Умный суффикс для адреса и часов работы
$lang_suffix = (isset($current_lang) && $current_lang != 'et') ? '_' . $current_lang : '';

$display_address = get_setting('footer_address' . $lang_suffix);
if(empty($display_address)) { $display_address = get_setting('footer_address'); }

$display_hours = get_setting('contact_hours' . $lang_suffix);
if(empty($display_hours)) { $display_hours = get_setting('contact_hours'); }
?>
...
@media (max-width: 900px) {
    ...
    .contact-page-wrapper { padding-top: 100px; }
}
</style>
